...
class AddGaussianNoise(object):
    ...
    Args:
        ...
        std_range (tuple): Tuple of (min_std, max_std) for the noise standard deviation.
    """
    ...
    def __call__(self, img, lbl):
        ...
        Args:
            ...
            lbl (PIL Image): Label (unchanged).
        Returns:
            PIL Image: Noisy image.
            PIL Image: Unchanged label.
        """
        ...
        img_noisy = Image.fromarray((img_np * 255).astype(np.uint8))
        return img_noisy, lbl

    def __repr__(self):
        return self.__class__.__name__ + '(mean={0}, std_range={1})'.format(self.mean, self.std_range)


class ClassWiseMasking(object):

    def __init__(self, p=0.1, num_classes_to_keep=1, ignore_index=255):
        self.p = p
    ...
    def __call__(self, img, lbl):
        if random.random() < self.p:
            ...
            if len(allowed_classes_present) < self.num_classes_to_keep:
                # Select random classes to keep
                classes_to_keep = random.sample(classes_present.tolist(), self.num_classes_to_keep)
            else:
                classes_to_keep = random.sample(allowed_classes_present, self.num_classes_to_keep)
            # Create mask for the classes to keep
            mask = np.isin(lbl_np, classes_to_keep)
            # Convert image to numpy array
            img_np = np.array(img)
            # Apply mask to image
            img_np[~mask] = 0  # Set pixels not in classes_to_keep to black
            # Set masked labels to ignore_index
            lbl_np[~mask] = 0  # this is the unlabeled class
            # Convert back to PIL Images
            img_masked = Image.fromarray(img_np)
            lbl_masked = Image.fromarray(lbl_np)
            return img_masked, lbl_masked
        else:
            return img, lbl

    def __repr__(self):
        return f"{self.__class__.__name__}(p={self.p}, num_classes_to_keep={self.num_classes_to_keep}, ignore_index={self.ignore_index})"
...
class ExtRandomRotation(object):
    ...
    def __call__(self, img, lbl):
        ...
        return F.rotate(img, angle, self.resample, self.expand,
                        self.center), F.rotate(lbl, angle, self.resample, self.expand, self.center)
